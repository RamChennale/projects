...
public class ViewCorpusStockInOutPage {
	...
	public ViewCorpusStockInOutPage(WebDriver driver) {
		this.driver = driver;
	}

	public void viewCorpusStockInOut() {
		...
		accountingPortfAutoSugg.sendKeys(file.getAccountingTestData("accountingPortfAutoSugg"));
		...
		for (int i = 0; i < clientPortfolioWebElementList.size(); i++) {
			if (clientPortfolioWebElementList.get(i).getText()
					.equalsIgnoreCase(file.getAccountingTestData("selectaccountingPortfAutoSugg"))) {
